...
export const getGetJournalEntryForAnalysisQueryKey = (analysisId: number) =>
  [`/api/journal/for-analysis`, analysisId] as const;
...
>(
  ...
  const queryKey =
    ...
    getGetJournalEntryForAnalysisQueryKey(analysisId);
  const queryFn: QueryFunction<
    Awaited<ReturnType<typeof getJournalEntryForAnalysis>>
  > = ({ signal }) =>
...
/**
 * @summary Returns the first journal entry linked to a specific analysis
 */
...
>(options?: {
  ...
  TContext
> => {
  const mutationKey = ["updateDailySummarySettings"];
  const { mutation: mutationOptions, request: requestOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
