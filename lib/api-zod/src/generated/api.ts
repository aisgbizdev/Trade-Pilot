...
export const GetProgressionCatalogResponse = zod.object({
  ...
  completedGuideIds: zod
    .array(zod.string())
    .describe("Guide article IDs with a valid, non-revoked completion award."),
});
...
export const GetPendingTopupRequestsQueryParams = zod.object({
  status: zod
    ...
    .default(getPendingTopupRequestsQueryStatusDefault)
    .describe("Filter by review status; defaults to pending"),
  page: zod.coerce
    .number()
    .int()
    .default(getPendingTopupRequestsQueryPageDefault),
  limit: zod.coerce
    .number()
    .int()
    .default(getPendingTopupRequestsQueryLimitDefault),
});
 ...
 */
export const LoginWithGoogleNativeBody = zod.object({
  idToken: zod
    .string()
    .describe(
...
export const LoginWithGoogleNativeResponse = zod.object({
  user: zod.object({
    id: zod.number().int(),
    email: zod.string(),
    displayName: zod.string(),
 ...
 */
export const ReauthenticateWithGoogleBody = zod.object({
  idToken: zod
    .string()
    .describe("A FRESH Google ID token for the same account. Never logged."),
});

export const ReauthenticateWithGoogleResponse = zod.object({
  reauthToken: zod
    .string()
    .describe("Short-lived, single-use token for one sensitive operation."),
  expiresAt: zod.coerce.date(),
});
 ...
 */
export const loginWithAppleNativeBodyIdentityTokenMax = 8000;

export const loginWithAppleNativeBodyAuthorizationCodeMax = 2048;

export const loginWithAppleNativeBodyNonceMax = 512;

export const loginWithAppleNativeBodyGivenNameMax = 100;

export const loginWithAppleNativeBodyFamilyNameMax = 100;

export const LoginWithAppleNativeBody = zod.object({
  identityToken: zod
    .string()
    .max(loginWithAppleNativeBodyIdentityTokenMax)
    .describe(
      "Apple identity token (JWT) from the device's native Sign in with Apple SDK. Never logged.",
    ),
  authorizationCode: zod
    .string()
    .max(loginWithAppleNativeBodyAuthorizationCodeMax)
    .describe(
      "Apple authorization code from the same native sign-in. Never logged.",
    ),
  nonce: zod
    .string()
    .max(loginWithAppleNativeBodyNonceMax)
    .describe(
      "The RAW nonce the client generated before sending its SHA-256 hash to Apple. Never logged.",
    ),
  givenName: zod
    .string()
    .max(loginWithAppleNativeBodyGivenNameMax)
    .optional()
    .describe(
  ...
  familyName: zod
    .string()
    .max(loginWithAppleNativeBodyFamilyNameMax)
    .optional()
    .describe(
      "Optional, only present on the first authorization. Used only as a display-name candidate when creating a brand-new account.",
    ),
...
export const LoginWithAppleNativeResponse = zod.object({
  user: zod.object({
    id: zod.number().int(),
    email: zod.string(),
    displayName: zod.string(),
 ...
 */
export const reauthenticateWithAppleBodyIdentityTokenMax = 8000;

export const reauthenticateWithAppleBodyAuthorizationCodeMax = 2048;

export const reauthenticateWithAppleBodyNonceMax = 512;

export const ReauthenticateWithAppleBody = zod.object({
  identityToken: zod
    .string()
    .max(reauthenticateWithAppleBodyIdentityTokenMax)
    .describe(
      "A FRESH Apple identity token for the same account. Never logged.",
    ),
  authorizationCode: zod
    .string()
    .max(reauthenticateWithAppleBodyAuthorizationCodeMax)
    .describe(
      "A FRESH Apple authorization code from the same sign-in. Never logged.",
    ),
  nonce: zod
    .string()
    .max(reauthenticateWithAppleBodyNonceMax)
    .describe("The RAW nonce used for this fresh sign-in. Never logged."),
});

export const ReauthenticateWithAppleResponse = zod.object({
  reauthToken: zod
    .string()
    .describe("Short-lived, single-use token for one sensitive operation."),
  expiresAt: zod.coerce.date(),
...
export const DeleteAccountBody = zod
  .object({
    currentPassword: zod
      .string()
      .optional()
      .describe("Required for password accounts (`user.hasPassword = true`)."),
    reauthToken: zod
      .string()
      .optional()
      .describe("Required for Google-only accounts. Single-use, ≤5 min."),
  })
  .describe(
 ...
 */
export const sendNativePushTestResponseTargetedMin = 0;

export const sendNativePushTestResponseAcceptedMin = 0;

export const SendNativePushTestResponse = zod
  .object({
    targeted: zod
      .number()
      .int()
      .min(sendNativePushTestResponseTargetedMin)
      .describe("Number of registered mobile device tokens targeted"),
    accepted: zod
      .number()
      .int()
      .min(sendNativePushTestResponseAcceptedMin)
      .describe("Number of messages accepted by FCM"),
    failures: zod
      .array(zod.enum(["unregistered", "auth", "invalid", "network"]))
      .describe("Failure category for each message not accepted by FCM"),
  })
  .describe(
