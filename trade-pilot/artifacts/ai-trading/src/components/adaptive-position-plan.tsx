...
import { buildAdaptivePositionPlan, type AccountTier, type AdaptivePositionPlanResult, type AdaptiveSidePositionPlan } from "@/lib/adaptive-position-plan";
...
function formatNumber(value: number | null | undefined, lang: "en" | "id", maximumFractionDigits = 2): string {
  ...
  return new Intl.NumberFormat(lang === "id" ? "id-ID" : "en-US", { maximumFractionDigits }).format(value);
}

function AdaptiveInput({
  label, value, onChange, placeholder, testId, step = "any",
}: {
  label: string;
  value: string;
  ...
  return (
    <label className="space-y-1">
      ...
      <Input type="number" min="0" step={step} value={value} placeholder={placeholder} onChange={(event) => onChange(event.target.value)} className="h-8 text-xs" data-testid={testId} />
    </label>
  );
}
...
function PlanSide({ plan, lang, copy }: { plan: AdaptiveSidePositionPlan; lang: "en" | "id"; copy: AdaptiveCopy }) {
  ...
  return (
    <div className={`rounded-md border-l-4 p-3 space-y-3 bg-muted/20 ${isBuy ? "border-l-emerald-500" : "border-l-red-500"}`} data-testid={`adaptive-plan-${plan.side}`}>
      ...
      <div className="space-y-1.5">{plan.ladder.map((level) => (
        <p key={`reason-${plan.side}-${level.level}`} className="text-[10px] text-muted-foreground leading-snug">
          <span className="font-semibold text-foreground/80">{level.level === 0 ? copy.adaptive_initial : `L${level.level}`}:</span> {level.reason}
        </p>
      ))}</div>
    </div>
  );
      ...
      {open && <div className="border-t border-border p-4 space-y-4" data-testid="adaptive-plan-content">
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            ...
            <label className="space-y-1"><span className="text-[11px] font-medium text-muted-foreground">{copy.adaptive_account_tier}</span>
              <select value={form.accountTier} onChange={(event) => updateField("accountTier", event.target.value as AccountTier)} className="flex h-8 w-full rounded-md border border-input bg-transparent px-2 text-xs" data-testid="select-adaptive-account-tier">
                <option value="micro">{copy.adaptive_tier_micro}</option><option value="mini">{copy.adaptive_tier_mini}</option><option value="regular">{copy.adaptive_tier_regular}</option>
              </select>
            </label>
          </div>
        </div>
        ...
        {!result && <p className="text-[11px] text-muted-foreground flex items-start gap-1.5"><Info className="w-3.5 h-3.5 mt-0.5 shrink-0" />{copy.adaptive_ready}</p>}
        {result && !result.valid && <div className="border border-amber-300 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/20 rounded-md p-3 space-y-2" data-testid="adaptive-plan-invalid">
          ...
          <ul className="list-disc pl-5 space-y-1 text-[11px] text-amber-800 dark:text-amber-300">{result.errors.map((error) => <li key={error}>{error}</li>)}</ul>
        </div>}
        {result?.valid && result.buy && result.sell && <div className="space-y-3" data-testid="adaptive-plan-valid">
          <Badge className="bg-emerald-600 hover:bg-emerald-600">{copy.adaptive_valid}</Badge>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3"><PlanSide plan={result.buy} lang={lang} copy={copy} /><PlanSide plan={result.sell} lang={lang} copy={copy} /></div>
          <div className="space-y-1.5 rounded-md border border-border p-3"><p className="text-xs font-bold text-foreground">{copy.adaptive_assumptions}</p><ul className="list-disc pl-5 space-y-1 text-[11px] text-muted-foreground">{result.assumptions.map((assumption) => <li key={assumption}>{assumption}</li>)}</ul></div>
