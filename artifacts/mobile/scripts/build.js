...
const DEFAULT_BUILD_METRO_PORT = 8082;
const BUILD_METRO_PORT_SEARCH_LIMIT = 20;
...
async function findAvailableMetroPort() {
  for (
    let port = DEFAULT_BUILD_METRO_PORT;
    port < DEFAULT_BUILD_METRO_PORT + BUILD_METRO_PORT_SEARCH_LIMIT;
    port++
  ) {
    if (await canListenOnPort(port)) return port;
  }
  throw new Error(
    `Could not find an available Metro build port in ${DEFAULT_BUILD_METRO_PORT}-${DEFAULT_BUILD_METRO_PORT + BUILD_METRO_PORT_SEARCH_LIMIT - 1}`,
  );
}
...
function metroUrl(pathname = "") {
  ...
  return `http://127.0.0.1:${metroPort}${pathname}`;
}

async function checkMetroHealth() {
  if (metroPort == null) return false;
  try {
...
async function startMetro(expoPublicDomain, expoPublicReplId) {
  ...
  console.log(`Starting Metro on isolated build port ${metroPort}...`);
