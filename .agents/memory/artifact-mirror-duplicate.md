...
**How to apply:** Validate and publish the root artifacts. Treat `trade-pilot/` workflow status as duplicate/stale unless explicitly requested. Stop all mirrored services first. Managed mirror workflows cannot be removed directly while their artifact metadata is registered; removing the mirror metadata from artifact discovery lets Replit prune them automatically. Then restart every canonical service that previously competed for a port—workflow “running” alone is insufficient; verify listeners or health endpoints. Use a fresh browser context or clear site data/service workers.